...
<div data-reveal class="reveal flex flex-col gap-2 p-4 rounded-lg bg-white shadow-sm hover:shadow-md transition">
    <div class="flex items-start justify-between">
        ...
        <div class="text-sm text-gray-700">Total: <strong
                class="text-gray-900">Rp{{ number_format($order->total, 0, ',', '.') }}</strong></div>
    </div>

    <div class="flex items-center justify-between">
        <div class="text-sm">
            <span
                class="badge {{ $order->status == 2 ? 'badge-warning' : ($order->status == 3 ? 'badge-info' : ($order->status == 4 ? 'badge-success' : 'badge-secondary')) }}">
                {{ $order->status_label }}
            </span>
        </div>
        ...
        @foreach ($order->orderItems as $item)
            ...
            <div class="flex items-center justify-between border rounded p-3">
                <div>
                    ...
                    @if ($item->shipped_at)
                        <div class="text-xs text-green-700">Dikirim: {{ $item->shipped_at->format('Y-m-d') }}
                ...
                <div>
                    ...
                    @if ($product && $canReview && !$alreadyReviewed)
                        <a href="{{ route('products.show', $product) }}#reviews"
                            class="px-3 py-2 bg-indigo-600 text-white rounded-md text-sm">Beri Ulasan</a>
                    @elseif($product && $canReview && $alreadyReviewed)
                        <a href="{{ route('products.show', $product) }}#reviews"
                            class="px-3 py-2 border rounded-md text-sm">Lihat Ulasan</a>
                    @else
                    ...
                    @if ($order->payment_method === 'bank_transfer' && $order->status === \App\Models\Order::STATUS_UNPAID)
                        <div class="mt-2">
                            <a href="{{ route('buyer.orders.payment', $order) }}"
                                class="px-3 py-2 bg-yellow-500 text-white rounded-md text-sm">Lihat Pembayaran</a>
                        </div>
                    @endif
                </div>
            </div>
