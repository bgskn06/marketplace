...
<tr parent-data="{{ $data }}" controller="{{ $controller }}" func="{{ $function }}" data-rowspan="{{ $rowspan - 1 }}" {{ $attributes }}>
...
@push('script')
    <script>
        ...
        function openAccordion(btn, event) {
            ...
            let controller = $(btn).closest('tr').attr('controller');
            let func = $(btn).closest('tr').attr('func');
            let rawData = $(btn).closest('tr').attr('parent-data');
        ...
        function domOpenAccordion(response) {
            const callbackMethod = '{{ $callback }}';
            const $btn = $(element);
            const $row = $(`tr[parent-data="${parentId}"]`);
