<div>
    ...
    <ul class="nav nav-tabs mb-4">
        ...
        <li class="nav-item">
            <a class="nav-link {{ $activeTab == 2 ? 'active font-weight-bold text-primary border-top-primary' : 'text-muted' }}" 
               href="#" wire:click.prevent="setTab(2)">
               Perlu Dikirim <i class="fas fa-box ml-1"></i>
            </a>
        </li>
        {{-- Tab 3: Shipped --}}
        <li class="nav-item">
            <a class="nav-link {{ $activeTab == 3 ? 'active font-weight-bold text-primary border-top-primary' : 'text-muted' }}" 
               href="#" wire:click.prevent="setTab(3)">
               Dikirim <i class="fas fa-truck ml-1"></i>
            </a>
        </li>
        {{-- Tab 4: Completed --}}
        <li class="nav-item">
            <a class="nav-link {{ $activeTab == 4 ? 'active font-weight-bold text-primary border-top-primary' : 'text-muted' }}" 
               href="#" wire:click.prevent="setTab(4)">
               Selesai
            </a>
        </li>
        {{-- Tab 0 & 5: Gagal --}}
        <li class="nav-item">
            <a class="nav-link {{ in_array($activeTab, [0, 5]) ? 'active font-weight-bold text-danger border-top-danger' : 'text-muted' }}" 
               href="#" wire:click.prevent="setTab(0)">
               Batal / Refund
            </a>
        </li>
    </ul>
                    ...
                    <div class="card-body">
                                    ...
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div class="mr-2 rounded overflow-hidden bg-white border" style="width: 40px; height: 40px;">
                                                @php 
                                                    // Ambil gambar (sesuaikan path polimorfik/biasa Anda)
                                                    $img = $item->product->mainPhoto 
                                                        ? asset('storage/'.$item->product->mainPhoto->path) 
                                                        : null; 
                                                @endphp
                                                <img src="{{ $img ?? 'https://via.placeholder.com/40' }}" class="w-100 h-100" style="object-fit: cover;">
                                            </div>
                                            <div>
                                                <h6 class="mb-0 font-weight-bold small text-dark">{{ $item->product->name }}</h6>
                        ...
                        <div class="d-flex justify-content-between align-items-center mt-3 pt-2 border-top">
                            ...
                            <div>
                                ...
                                @if($order->status == 2)
                                    <button wire:click="openResiModal({{ $order->id }})" 
                                            class="btn btn-primary btn-sm px-4"
                                            data-toggle="modal" 
                                            data-target="#modalResi">
