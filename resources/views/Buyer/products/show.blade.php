                            ...
                            <div class="mt-6">
                                ...
                                <div class="flex flex-wrap items-center gap-3">
                                    ...
                                    <form action="{{ route('chat.start', $product->id) }}" method="POST">
                                        @csrf
                                        <button type="submit" class="px-4 py-2 border border-indigo-600 text-indigo-600 rounded-md hover:bg-indigo-50 flex items-center gap-2 transition">
                                            {{-- Ikon Chat SVG --}}
                                    ...
                                    <button disabled class="px-4 py-2 border border-gray-300 text-gray-400 rounded-md cursor-not-allowed flex items-center gap-2">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                    ...
                                    <a href="{{ route('login') }}" class="px-4 py-2 border border-indigo-600 text-indigo-600 rounded-md hover:bg-indigo-50 flex items-center gap-2 transition">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M20.25 8.511c.884.284 1.5 1.128 1.5 2.097v4.286c0 1.136-.847 2.1-1.98 2.193-.34.027-.68.052-1.02.072v3.091l-3-3c-1.354 0-2.694-.055-4.02-.163a2.115 2.115 0 0 1-.825-.242m9.345-8.334a2.126 2.126 0 0 0-.476-.095 48.64 48.64 0 0 0-8.048 0c-1.186.078-2.1 1.075-2.1 2.2v4.286c0 .197.03.388.077.57m0 0a5.026 5.026 0 0 1-1.097-.076 3.016 3.016 0 0 0-.75.408l-2.146 2.147a.5.5 0 0 1-.778-.363V15.93c-1.136-.092-1.98-1.057-1.98-2.193v-4.286c0-.97.616-1.813 1.5-2.097" />
                                        </svg>
                                        Chat Login
                                    ...
                                    <form method="POST" action="{{ route('buyer.cart.add') }}">
                                        ...
                                        <button type="submit" class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 shadow-sm flex items-center gap-2 transition">
                                            {{-- Ikon Keranjang --}}
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                ...
                                <div id="reviews" class="mt-6 bg-white rounded-lg shadow p-4">
                                    ...
                                    <div class="mb-3">
                                        <div class="text-sm text-yellow-500">
                                            {{ number_format($product->rating ?? 0, 1) }} ★</div>
                                    </div>
                                    ...
                                    <form method="POST" action="{{ route('buyer.products.reviews.store', $product) }}" class="space-y-2">
                                        ...
                                        <div>
                                            ...
                                            <select name="rating" class="border-gray-200 rounded-md p-2 text-sm">
                                                @for ($i = 5; $i >= 1; $i--)
                                                <option value="{{ $i }}" {{ (old('rating') ?? ($existingReview->rating ?? '')) == $i ? 'selected' : '' }}>
                                                    {{ $i }} ★</option>
                                                @endfor
                                            </select>
                                        </div>
                                        ...
                                        <div>
                                            <button type="submit" class="px-4 py-2 bg-indigo-600 text-white rounded-md">Kirim
                                                Ulasan</button>
                                        </div>
                                    </form>
                                    @else
                                    <div class="text-sm text-gray-600">Anda dapat memberi rating setelah pesanan
                                        diterima.</div>
                                    @endif
                                    @else
                                    ...
                                    <div class="mt-4 space-y-3">
                                        ...
                                        <div class="border rounded p-3">
                                            <div class="text-sm font-medium text-gray-800">
                                                {{ $review->user->name ?? 'User' }} • <span class="text-yellow-500">{{ $review->rating }} ★</span></div>
